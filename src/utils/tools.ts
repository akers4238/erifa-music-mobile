...
import { pushWebviewScreen } from '@/navigation/navigation'
...
export const openUrl = async(url: string): Promise<void> => {
  if (/^https?:\/\//i.test(url) && commonState.componentIds.home) {
    pushWebviewScreen(commonState.componentIds.home, url)
    return
  }
  return Linking.canOpenURL(url).then(async() => Linking.openURL(url))
}

const builtinOnlineSources = new Set<string>(
  (musicSdk.sources as Array<{ id: string }>).map(source => source.id),
)

export const assertApiSupport = (source: LX.Source): boolean => {
  return source == 'local' || global.lx.qualityList[source] != null || builtinOnlineSources.has(source)
}
...
export const cheatTip = async() => {
  ...
  return tipDialog({
    title: '谨防被骗提示',
    message: `1. 本项目无微信公众号之类的所谓「官方账号」，也未在小米、华为、vivo 等应用商店发布应用，商店内的「Music」相关的应用全部属于假冒应用，谨防被骗！\n
